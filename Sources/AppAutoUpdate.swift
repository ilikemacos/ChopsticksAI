...
final class AppAutoUpdate: ObservableObject {
    ...
    func configure(_ config: AppUpdateConfig) {
        ...
        if UserDefaults.standard.object(forKey: config.autoInstallKey) != nil {
            ...
            autoInstall = true
            UserDefaults.standard.set(true, forKey: config.autoInstallKey)
        }
    }
    ...
    private func performInstall(config: AppUpdateConfig, version: String, zipName: String, expectedSha: String?) -> InstallOutcome {
        ...
        return Self.scheduleReplace(staged: staged, dest: dest, productName: config.productName, version: version)
    }

    private static func scheduleReplace(staged: URL, dest: URL, productName: String, version: String) -> InstallOutcome {
        let script = """
        #!/bin/bash
        set -e
        sleep 1
        ...
        codesign --force --sign - '\(shellQuote(dest.path))' 2>/dev/null || true
    ...
    private static func installDestination(bundleName: String) -> URL {
        ...
        let homeApps = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Applications/\(bundleName)", isDirectory: true)
        if running.path.hasPrefix(homeApps.deletingLastPathComponent().path) { return running }
        if running.path.hasPrefix("/Applications/") { return running }
        return homeApps
    }

    private static func isAllowedDestination(_ url: URL) -> Bool {
        let p = url.path
        return p.contains("/Applications/") && !p.contains("AppTranslocation")
    }
