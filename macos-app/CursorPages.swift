...
struct AccountView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            PageHeader(
                title: "Account",
                subtitle: "Sign in to save chats to Supabase. Guests keep chats on this Mac only."
            )
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    if auth.isSignedIn {
                        SettingsCard(title: "Signed in", subtitle: auth.email) {
                            Text("Your agent chats sync to your account after each reply.")
                        ...
                        SettingsCard(title: "Sign in or create account", subtitle: "Email + password via Supabase Auth.") {
                    ...
                    SettingsCard(title: "Privacy", subtitle: "Optional cloud sync") {
                        Text("Chats are stored in your Supabase account with row-level security — only you can read them. Without an account, chats stay in memory on this device.")
                            .font(.system(size: 12.5))
                            .foregroundStyle(Cursor.muted)
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                    pluginCard(name: "Mozilla Search", desc: "MDN · Wikipedia · DuckDuckGo — Search rail.", installed: true)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                case .agents:
                    SettingsCard(title: "Agent behavior") {
                        ...
                        SettingsToggleRow(
                            title: "Web search",
                            subtitle: "Automatic Mozilla lookup on each question. Off = faster model-only replies; `/search …` still forces lookup.",
                ...
                case .network:
                    SettingsCard(title: "Network") {
                        SettingsToggleRow(
                            title: "Web search",
                            subtitle: "Mozilla engine (MDN, Wikipedia, DuckDuckGo). Turn off to skip lookups and save time.",
                            isOn: Binding(get: { store.webSearchEnabled }, set: { store.setWebSearchEnabled($0) })
                        )
                        Divider().overlay(Cursor.hairline)
                        Text("API endpoint")
                ...
                case .privacy:
                    SettingsCard(title: "Privacy Mode") {
                        SettingsToggleRow(
                            title: "Privacy Mode",
                            subtitle: "Prefer local KB when possible; avoid storing chat content remotely beyond the live request.",
