...
enum StorageUpload {
    ...
    private static func storageObjectURL(path: String) -> URL? {
        ...
        return URL(string: "\(CloudPublic.url.absoluteString)/storage/v1/object/\(AttachLimits.bucket)/\(enc)")
    }
    ...
    private static func sign(path: String, token: String) async throws -> String {
        ...
        guard let url = URL(string: "\(CloudPublic.url.absoluteString)/storage/v1/object/sign/\(AttachLimits.bucket)/\(enc)") else {
        ...
        return CloudPublic.url.appendingPathComponent("storage/v1").absoluteString
    ...
    private static func tusUpload(
        ...
        guard let endpoint = URL(string: "\(CloudPublic.url.absoluteString)/storage/v1/upload/resumable") else {
        ...
        create.setValue(CloudPublic.anonKey, forHTTPHeaderField: "apikey")
        ...
        if let abs = URL(string: loc), abs.scheme != nil {
            uploadURL = abs
        } else if let rel = URL(string: loc, relativeTo: CloudPublic.url)?.absoluteURL {
            uploadURL = rel
        } else {
        ...
        while offset < data.count {
            ...
            patch.setValue(CloudPublic.anonKey, forHTTPHeaderField: "apikey")
